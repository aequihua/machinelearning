...
```{r predict_first, echo=TRUE, warning=FALSE, message=FALSE,cache=FALSE}
  ...
  confusionMatrix(wltrain2$classe, preds)
```

It can be seen that the accuracy of this model is `r fit$results$Accuracy` , which is considered adequate. So it is decided to keep this modelling method to predict the values of the test data.

Now it is time to retrain the model with the same random forest method, using the entire training dataset, so that we use this model for the actual predictions of the test dataset:
